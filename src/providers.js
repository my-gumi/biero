// LLM provider registry — provider-agnostic, modeled after hermes-agent's
// `providers/` layer. Each entry connects to an OpenAI-compatible endpoint
// unless a different `strategy` is given.
...
// `/models` endpoints return these mixed in with chat models — filter them out.
const NON_CHAT = [
 ...
 * Provider-agnostic: keeps anything that isn't obviously a non-chat model,
 * ranks known families first, and pushes dated snapshots below their alias.
 */
export function curateModels(provider, ids = []) {
  const families = FAMILIES[provider?.id] ?? [];
  const chat = ids.filter((id) => {
    const l = String(id).toLowerCase();
    return l && !NON_CHAT.some((k) => l.includes(k));
  });
  const rank = (id) => {
 ...
 * Returns { ok, models?, status?, message? }.
 */
export async function validateLLM(provider, apiKey, baseURL) {
